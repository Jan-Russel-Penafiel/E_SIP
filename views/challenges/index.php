...
<div class="mb-8">
    ...
    <p class="text-gray-300 text-sm font-bold" style="text-shadow: 1px 1px 0px #333333;">Test your skills with interactive coding challenges and quizzes.</p>
</div>
...
<div class="flex flex-wrap gap-3 mb-6">
    ...
    <div class="relative">
        <select onchange="window.location.href=this.value"
            class="appearance-none bg-surface border-4 border-gray-600 rounded-none px-4 py-2 pr-10 text-sm text-white font-bold focus:outline-none cursor-pointer" style="box-shadow: inset 4px 4px 0px #555, inset -4px -4px 0px #fff;">
...
<div class="text-center py-16">
    ...
    <p class="text-gray-300 font-bold" style="text-shadow: 1px 1px 0px #333333;">No challenges found.</p>
</div>
<?php else: ?>
<div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
    ...
    ?>
     <a href="<?= $baseUrl ?>/challenges/<?= $challenge['id'] ?>"
         class="group bg-surface border-4 border-gray-600 rounded-none p-5 card-hover transition-all duration-200 <?= $isCompleted ? 'opacity-75' : '' ?>" style="box-shadow: inset 4px 4px 0px #c6c6c6, inset -4px -4px 0px #555555;">
        <!-- Status Badge -->
        <div class="flex items-center justify-between mb-3">
            <span class="flex items-center gap-1.5 text-sm font-bold" style="text-shadow: 1px 1px 0px #333333;">
                <span style="filter: drop-shadow(1px 1px 0px #333333);"><?= $langInfo['icon'] ?></span>
                <span class="text-gray-300 lang-label"><?= $langInfo['name'] ?></span>
            </span>
            <?php if ($isCompleted): ?>
            <span class="text-xs text-green-400 font-bold flex items-center gap-1" style="text-shadow: 1px 1px 0px #333333;">
        ...
        <p class="text-sm text-gray-300 mb-3 line-clamp-2 font-bold" style="text-shadow: 1px 1px 0px #333333;"><?= htmlspecialchars($challenge['description']) ?></p>

        <div class="flex items-center gap-2 text-xs font-bold" style="text-shadow: 1px 1px 0px #333333;">
            <span class="px-2 py-1 bg-surface text-gray-300 border-2 border-gray-600" style="box-shadow: inset 2px 2px 0px #333333, inset -2px -2px 0px #fff;"><?= ucfirst($challenge['difficulty']) ?></span>
            <span class="px-2 py-1 bg-surface text-gray-300 border-2 border-gray-600" style="box-shadow: inset 2px 2px 0px #333333, inset -2px -2px 0px #fff;"><?= ucfirst($challenge['type']) ?></span>
