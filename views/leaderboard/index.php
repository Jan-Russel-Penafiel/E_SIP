...
<div class="mb-8">
    ...
    <p class="text-gray-300 text-sm font-bold" style="text-shadow: 1px 1px 0px #000;">See how you rank against other learners on the platform.</p>
</div>

<!-- Your Rank Card -->
<div class="bg-surface border-4 border-black rounded-none p-6 mb-6" style="box-shadow: inset 4px 4px 0px #c6c6c6, inset -4px -4px 0px #555555;">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="text-center">
            ...
            <p class="text-sm text-gray-300 font-bold" style="text-shadow: 1px 1px 0px #000;">Your current ranking</p>
        </div>
        <div class="flex items-center gap-8">
            <div class="text-center">
                <p class="text-3xl font-bold text-white" style="text-shadow: 2px 2px 0px #3f3f3f;">#<?= $userRank ?: '-' ?></p>
                <p class="text-xs text-gray-300 font-bold" style="text-shadow: 1px 1px 0px #000;">Rank</p>
            </div>
            <div class="text-center">
                <p class="text-2xl font-bold text-white" style="text-shadow: 2px 2px 0px #3f3f3f;"><?= $user['level'] ?></p>
                <p class="text-xs text-gray-300 font-bold" style="text-shadow: 1px 1px 0px #000;">Level</p>
            </div>
            <div class="text-center">
                <p class="text-2xl font-bold text-white" style="text-shadow: 2px 2px 0px #3f3f3f;"><?= $user['xp'] ?></p>
                <p class="text-xs text-gray-300 font-bold" style="text-shadow: 1px 1px 0px #000;">XP</p>
            </div>
            <div class="text-center">
...
<div class="bg-surface border-4 border-black rounded-none overflow-hidden" style="box-shadow: inset 4px 4px 0px #c6c6c6, inset -4px -4px 0px #555555;">
    ...
    <div class="text-center py-16">
        ...
        <p class="text-gray-300 font-bold" style="text-shadow: 1px 1px 0px #000;">No rankings yet. Complete challenges to get on the leaderboard!</p>
    </div>
    <?php else: ?>
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody>
                ...
                ?>
                <tr class="border-b-4 border-black last:border-0 hover:bg-gray-600 transition <?= $isCurrentUser ? 'bg-gray-700' : '' ?>">
                    <td class="px-6 py-4 text-lg font-bold text-center" style="filter: drop-shadow(2px 2px 0px #000);"><?= $rankIcon ?></td>
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <span class="font-bold pixel-font <?= $isCurrentUser ? 'text-white' : 'text-gray-300' ?>" style="text-shadow: 1px 1px 0px #000;">
                                <?= htmlspecialchars($player['username']) ?>
                                <?= $isCurrentUser ? ' (You)' : '' ?>
                            </span>
                        </div>
                    </td>
                    <td class="px-6 py-4 text-center text-sm font-bold" style="text-shadow: 1px 1px 0px #000;"><?= $player['level'] ?></td>
                    <td class="px-6 py-4 text-center text-sm font-bold" style="text-shadow: 1px 1px 0px #000;"><?= $player['xp'] ?></td>
                    <td class="px-6 py-4 text-center text-sm font-bold" style="text-shadow: 1px 1px 0px #000;"><?= $player['challenges_completed'] ?></td>
                    <td class="px-6 py-4 text-center text-sm font-bold" style="text-shadow: 1px 1px 0px #000;">
